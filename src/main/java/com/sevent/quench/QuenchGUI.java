...
public class QuenchGUI {
    public void openQuenchGUI(Player player) {
        Inventory inventory = Bukkit.createInventory(null, QuenchConstants.INVENTORY_SIZE, QuenchConstants.QUENCH_GUI_TITLE);
        // 填充背景物品
    ...
    private void fillBackground(Inventory inventory) {
        ...
        // 填充背景
        for (int i = 0; i < QuenchConstants.INVENTORY_SIZE; i++) {
            if (i != QuenchConstants.EQUIPMENT_SLOT && i != QuenchConstants.CONFIRM_SLOT && i != QuenchConstants.CANCEL_SLOT && i!=QuenchConstants.QUENCH_STONE_SLOT) {
